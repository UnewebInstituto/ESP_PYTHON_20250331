...
import sys
if sys.path.count('C:\\ESP_PYTHON_20250331\\NIVEL_II\\HENRY') == 0:
    sys.path.append('C:\\ESP_PYTHON_20250331\\NIVEL_II\\HENRY')
...
def guardar_datos():    
    ...
    messagebox.showwarning('Guardar datos','Los datos fueron guardados con éxito')
...
def salir():
    resp = messagebox.askyesno('Salir','¿Está seguro que quiere abandonar la aplicación')
    if resp == True:
        ventana.quit()
    else:
        messagebox.showinfo('Salir','Se mantiene en ejecución la aplicación de guardar datos')
