...
-- ASCENDENTE POR OMISIÓN
SELECT * FROM VISTA_PROV_PROD ORDER BY PRODUCTO;

SELECT * FROM VISTA_PROV_PROD ORDER BY PRODUCTO DESC;

-- COMBINACIÓN DE ORDEN

SELECT * FROM VISTA_PROV_PROD ORDER BY PRODUCTO DESC, PRECIO ASC 
...
FROM VISTA_PROV_PROD GROUP BY PROVEEDOR;

-- UTILIZANDO AGRUPAMIENTO Y APLICANDO ALIAS:
SELECT PROVEEDOR, MAX(PRECIO) AS "Precio máximo", MIN(PRECIO) AS "Precio mínimo", ROUND(AVG(PRECIO),2) AS "Precio promedio", SUM(EXISTENCIA) AS "Cantidad en existencia" 
FROM VISTA_PROV_PROD GROUP BY PROVEEDOR;

-- UTILIZANDO AGRUPAMIENTO, APLICANDO ALIAS Y ORDENANDO POR EL NOMBRE DEL PROVEEDOR:
SELECT PROVEEDOR AS "Proveedor", MAX(PRECIO) AS "Precio máximo", MIN(PRECIO) AS "Precio mínimo", ROUND(AVG(PRECIO),2) AS "Precio promedio", SUM(EXISTENCIA) AS "Cantidad en existencia" 
